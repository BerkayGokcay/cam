...
# (İlk etapta mevcut 68-pt face_alignment ile; sonra 5-pt TRT'ye geçebiliriz)
...
job_q: "queue.Queue[Tuple[str,np.ndarray, int]]" = queue.Queue(maxsize=1024)  # (object_name, img, delivery_tag)
...
class ArcFaceONNX:
    def __init__(self, model_path: str, gpu_id: int = 0):
        ...
        ishape = self.session.get_inputs()[0].shape  # [N,3,H,W]
...
arc = ArcFaceONNX(os.getenv("ARCFACE_MODEL", "/opt/models/recognition/w600k_r50.onnx"), gpu_id=GPU_ID)
...
def process_pending(items: List[Tuple[str,np.ndarray,int]]):
    ...
    for name, a, dtag in zip(names, aligned, tags):
        if a is None:
            ...
            safe_remove(name)
            continue
        e = next(it, np.zeros((128,), np.float32))  # TODO: DB'ye yaz
...
# onnxruntime-gpu
# face-alignment
# torch  # (face_alignment bağımlılığı)
# torchvision
#
# Çalıştırma (örnek)
# ------------------
# export RMQ_URL=amqp://guest:guest@rabbitmq:5672/%2F
# export MINIO_HOST=minio; export MINIO_PORT=9000; export MINIO_USER=minioadmin; export MINIO_PASS=minioadmin
# python3 video_process.py /data/video.mp4
# python3 worker.py
